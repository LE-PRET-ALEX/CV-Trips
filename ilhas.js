// Carrosel de botoes Gastronomia 

//capturar elementos
const slide_carrosel = document.querySelectorAll('.carrosel_deslizar_gatronomia');
const btn_esq = document.getElementById('btn-ant-gastronomia'); //Botao anterior/esquerda
const btn_dir = document.getElementById('btn-next-gastronomia'); //Botao proximo/direita
...
function moverdireita(){
    ...
    carrosel_deslizar_gastronomia.style.transform = `translateX(-${ponto_inicial_slide}px)`;
...
function moveresquerda(){
    ...
    if(ponto_inicial_slide < 0){
        ...
        carrosel_deslizar_gastronomia.style.transform = `translatX(0px)`;
    }
}
...
function verificar_fim_carrosel(){
    const totaitems = document.querySelectorAll('.item-gastronomia').length;
    
    const posicao_maxima = (totaitems - 1) * deslocamento;

    if(ponto_inicial_slide > posicao_maxima){
        ponto_inicial_slide = 0;
        carrosel_deslizar_gastronomia.style.transform = `translateX(0px)`;
    }
}
